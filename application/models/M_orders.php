...
class M_orders extends CI_Model{
    ...
    function get_orders(){
        $hasil=$this->db->query("SELECT id_order,tanggal,idpaket,nama_paket,id_hotel,nama_hotel,rate,harga1,harga2,harga3,adult,

        CASE WHEN adult < 3 THEN adult*harga0 WHEN adult < 5 THEN adult*harga1 WHEN adult < 7 THEN adult*harga2 WHEN adult < 9 THEN adult*harga3 END AS total,
		CASE WHEN adult < 3 THEN harga0 WHEN adult < 5 THEN harga1 WHEN adult < 7 THEN harga2 WHEN adult < 9 THEN harga3 END AS tober,
        berangkat,metode,bank,norek,atasnama,nama,IF(jenkel='L','Laki-Laki','Perempuan')AS jenkel,alamat,notelp,email,keterangan,status FROM orders JOIN metode_bayar ON metode_id=id_metode JOIN pkt_hotel ON hotel_order_id=id_hotel JOIN paket ON paket_id_order=idpaket GROUP BY id_order order by tanggal desc");
        return $hasil;
    }
